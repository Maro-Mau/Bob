...
class LevelSystem(commands.Cog):
    ...
    async def check_user(self, user_id):
        async with aiosqlite.connect(self.DB) as db:
            await db.execute("INSERT OR IGNORE INTO Patient (user_id) VALUES (?)", (user_id,))
            await db.commit()

    async def get_xp(self, user_id):
        await self.check_user(user_id)
        async with aiosqlite.connect(self.DB) as db:
            async with db.execute("SELECT xp FROM Patient WHERE user_id = ?", (user_id,)) as cursor:
    ...
    async def on_message(self, message):
        ...
        async with aiosqlite.connect(self.DB) as db:
            await db.execute(
                "UPDATE Patient SET message_count = message_count + 1, xp = xp + ? WHERE user_id = ?", (
    ...
    async def rang(self, ctx):
        async with aiosqlite.connect(self.DB) as db:
            async with db.execute("SELECT xp FROM Patient WHERE user_id = ?", (ctx.author.id,)) as cursor:
                result = await cursor.fetchone()
                if result is None:
                    await ctx.respond("Wir Haben noch keine Krankenakte von dir", ephemeral=True)
                    return
                xp = result[0]
    ...
    async def leaderboard(self, ctx):
        async with aiosqlite.connect(self.DB) as db:
            async with db.execute("SELECT user_id, xp FROM Patient ORDER BY xp DESC LIMIT 10") as cursor:
